...
def part2(input_file: str):
    ...
    for i in range(max_col + 1):
        start_positions.append(Position(0, i, Direction.S))
        start_positions.append(Position(max_row, i, Direction.N))
    for i in range(max_row + 1):
        start_positions.append(Position(i, 0, Direction.E))
        start_positions.append(Position(i, max_col, Direction.W))
    result = 0
    for pos in start_positions:
        e_grid = energized_grid(grid, pos)
        result = max(result, count_energized_tiles(e_grid))
    return result
